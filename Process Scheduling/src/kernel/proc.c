 ...
 */
  ...
  if ( (rp = rdy_head[USER_Q]) != NIL_PROC) {
	proc_ptr = rp;
  /* check if process has batch type if so change bill_ptr so that it won't be preempted*/
	bill_ptr = rp->p_type == BATCH ? proc_addr(IDLE) : rp;
	return;
  }
 ...
 */
  register struct proc *prev, *curr;
  register struct proc *tmp1,*tmp2;
  if (istaskp(rp)) {
	if (rdy_head[TASK_Q] != NIL_PROC)
  ...
  /* Add user process to the end of appropriate group of processes */
  ...
  /* Check if new process has bigger priority than current head of queue */
  if (rdy_head[USER_Q]->p_type < rp->p_type) {
    rp->p_nextready = rdy_head[USER_Q];
    rdy_head[USER_Q] = rp;
    return;
  }

  /* Find last process which has bigger or equal priority than newly added process
  so that it can be appended to appropriate group */
  prev = NIL_PROC;
  curr = rdy_head[USER_Q];
  while (curr && curr->p_type >= rp->p_type) {
    prev = curr;
    curr = curr->p_nextready;
  }
  rp->p_nextready = curr;
  prev->p_nextready = rp;
  if (!curr)  rdy_tail[USER_Q] = rp;
 ...
 * possibly promoting another user to head of the queue.*/
   ...
   register struct proc *prev, *curr, *rp;
  rp = curr = rdy_head[USER_Q];
  prev = NIL_PROC;

  /* Check if queue is empty */
  if (curr == NIL_PROC) return;

  if (curr->p_type == INTERACTIVE) {
    /* Finds first non interactive process */
    while (curr && curr->p_type == INTERACTIVE) {
      prev = curr;
      curr = curr->p_nextready;
    }
    /* First process is appended to the last interactive process */
    if (prev != rp) {
  ...
  else if (curr->p_type == NORMAL) {
    register struct proc *tprev, *tcurr;
    register struct proc *tmp;
    tcurr = rp;
    /* Find process with biggest priority and increment ACT_PRI if necessary */
    while (curr && curr->p_type == NORMAL) {
      if (curr->ACT_PRI < MAX_AGE) 
        (curr->ACT_PRI)++;
      if (tcurr->ACT_PRI < curr->ACT_PRI) {
        tcurr = curr;
        tprev = prev;
      }
      prev = curr;
    ...
    /* Swap process from head of queue with process with biggest priority */
    if (tprev != rp) {
      tmp = tcurr->p_nextready;
      tcurr->p_nextready = rp->p_nextready;
      rp->p_nextready = tmp;
      tprev->p_nextready = rp;
      rdy_head[USER_Q] = tcurr;

      if (!curr)  rdy_tail[USER_Q] = rp;
    }
  }
  /* OLD MINIX SCHEDULING

...
/* Start the ball rolling. */
  ...
  for (rp = BEG_PROC_ADDR, t = -NR_TASKS; rp < END_PROC_ADDR; ++rp, ++t) {
	rp->p_nr = t;		/* proc number from ptr */
		...
		rp->p_type = INTERACTIVE;
		rp->BASE_PRI = rp->ACT_PRI = MIN_PRI;
  }
  ...
  proc[NR_TASKS+INIT_PROC_NR].p_type = INTERACTIVE;
  proc[NR_TASKS+INIT_PROC_NR].BASE_PRI = MIN_PRI;
  proc[NR_TASKS+INIT_PROC_NR].ACT_PRI = MIN_PRI;
